...
//				num = sc.nextInt();
//				sc.nextLine();          ///////////nextInt()에서 마지막에 enter가 남아있기때문에 nextLine으로 먹어준다  nextLine은 enter next는 공백을 먹는다
...
//		}
		...
		//크기가 4인 1차원배열 3개를 담고있는 2차열 배열을 만들어라 자료형 int
//		 int[][] arr = new int[3][4];
...
//		 }
		...
		//크기가 각각 4 3 5 2 인 1차원배열들로 생성된 2차원배열을 만들고 값을 전부 10으로 초기화
...
//		for(int i = 0; i < arr.length; i++) {			
//			for(int j = 0; j < arr[i].length; j++) {				
//				arr[i][j] = ch++;                                     //////값 다르게 나옴이 아니라 위에서 읽다보면 ch++ 두번씩 돼서 
//				System.out.print(arr[i][j] +" ");	               //////////a c e순으로 하나 걸러 나온거엿음
//				if(j==arr[i].length-1)
//					System.out.println();
//			}
//	}
...
//		int[][] arr = new int[4][4];
//		for(int i = 0; i < arr.length; i++) {			
//			for(int j = 0; j < arr[i].length; j++) {				        //줄인거랑 두개  for 쓰는거랑 다른가???
//				arr[i][j] = num++;				
//					System.out.print(arr[i][j]+" ");
//					
//						
